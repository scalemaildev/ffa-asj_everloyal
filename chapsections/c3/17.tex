...
\begin{itemize}
\item \textbf{Every Round:} \emph{Eóghainn steps into the fog.} Roll 3D6. Calculate a hex coordinate (X,Y) from any two sums of these three dice, and place Zealot Eóghainn on that tile. The chosen location must be a free and valid hex. If no combination is valid, then re-roll. Remove all condition tokens (including Knockdown) from his enemy sheet before moving.
\end{itemize}
