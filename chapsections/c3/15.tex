...
\begin{itemize}
\item \textbf{Every 3rd Round:} \emph{Lukso screams, and its body pulses with a menacing glow.} For this Round, the \emph{Belch Fire} attack is replaced by \emph{Roaring Jet}.
\end{itemize}
...
\begin{tabular}{s s s}
\textbf{HP:} 35 & \textbf{Move:} 2\\
\textbf{P.DEF:} 2 & \textbf{M.DEF:} 1 & \textbf{D.DEF:} 1 \\
\textbf{F.DEF:} 3 \\
\end{tabular}\\
...
\textbf{Attacks:}
\begin{itemize}
\item \emph{Swipe} - Move 1. Deal 3 \emph{Unparryable} Slash damage to an adjacent entity.
\item \emph{Lunge} - Move 2. Deal 2 \emph{Unparryable} Crush damage and inflict Knockback 1 on an adjacent entity.
\item \emph{Choke Slam} - Deal 3 \emph{Unblockable} Crush damage and inflict Knockdown on an adjacent entity.
\item \emph{Belch Fire} - Deal 3 \emph{Undodgeable} Burn ranged damage to an entity within 2 tiles.
...
\begin{tcolorbox}
\textbf{Hint:} While \emph{Belch Fire} and \emph{Roaring Jet} are \emph{Undodgeable}, that only means that the attack’s damage cannot be mitigated by \emph{Dodge!} The implicit Move of \emph{Dodge!} can still take the character out of an attack’s range, and reactions such as Take Cover and \emph{Dive!} can also be used to avoid ranged damage.
\end{tcolorbox}
